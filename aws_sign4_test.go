...
func TestVersion4RequestPreparer_2(t *testing.T) {
	request := test_plainRequestV4(false)
	actualSigned := calculateAWSv4Signature(request, *testCredV4)
	...
	// TODO
	//assert.So(actual, should.Equal, "AWS4-HMAC-SHA256 Credential=AKIDEXAMPLE/20190416/us-east-1/iam/aws4_request, SignedHeaders=content-type;host;x-amz-content-sha256;x-amz-date, Signature=e431678d1014e4f620542b90c78436526682210d3a44787a128d8d4baf104ae7")
}
...
func TestVersion4SigningTasks(t *testing.T) {
	...
	signer := newV4Signer(request, *testCredV4)
	signature := signer.calculateSignature()
	assert := assertions.New(t)

	// (Task 1) The canonical request should be built correctly
	assert.So(signature.task1_HashedCanonicalRequest, should.Equal, expectedV4["CanonicalHash"])

	// (Task 2) The string to sign should be built correctly
	assert.So(signature.task2_StringToSign, should.Equal, expectedV4["StringToSign"])

	// (Task 3) The version 4 signed signature should be correct
	assert.So(signature.task3_Signature, should.Equal, expectedV4["SignatureV4"])

	// (Task 4) The header should be composed correctly
	//assert.So(signature.task4_AuthHeader, should.Equal, expectedV4["AuthHeader"]) // TODO:?
}
...
func TestSignature4Helpers_1(t *testing.T) {
	...
	expected := expectedV4["AuthHeader"] + expectedV4["SignatureV4"]
	actual := task4_AuthHeader(expectedV4["SignatureV4"], meta, *testCredV4)
...
func TestSignature4Helpers_3(t *testing.T) {
	...
	assertions.New(t).So(timestampDateV4(ts), should.Equal, "20110909")
}
...
func test_plainRequestV4(trailingSlash bool) *http.Request {
	...
	return httptest.NewRequest("POST", address, body)
}
...
var (
	testCredV4 = &awsCredentials{
		AccessKeyID:     "AKIDEXAMPLE",
		SecretAccessKey: "wJalrXUtnFEMI/K7MDENG+bPxRfiCYEXAMPLEKEY",
	}

	testCredV4WithSTS = &awsCredentials{
		AccessKeyID:     "AKIDEXAMPLE",
		SecretAccessKey: "wJalrXUtnFEMI/K7MDENG+bPxRfiCYEXAMPLEKEY",
